...
namespace BLE_tcp_driver
{
    ...
    static class ProtocolHelper
    {
        ...
        {
            return BuildDeviceStatusPacket(PacketType.DeviceInfoResp, info);
        }

        public static byte[] BuildLiveDeviceStatusPacket(DeviceStatusInfo info, byte[] requestId)
        {
            byte[] statusPacket = BuildDeviceStatusPacket(PacketType.LiveDeviceStatusResp, info);
            byte[] statusData = new byte[statusPacket.Length - 3];
            Array.Copy(statusPacket, 3, statusData, 0, statusData.Length);
            byte[] data = new byte[requestId.Length + statusData.Length];
            Array.Copy(requestId, 0, data, 0, requestId.Length);
            Array.Copy(statusData, 0, data, requestId.Length, statusData.Length);
            return BuildPacket(PacketType.LiveDeviceStatusResp, data);
        }

        private static byte[] BuildDeviceStatusPacket(PacketType type, DeviceStatusInfo info)
        {
